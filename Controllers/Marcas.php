...
class Marcas extends Controller{
	...
	public function listar()
	{
		...
		for ($i=0; $i < count($data); $i++) {
			...
			$data[$i]['acciones'] = '<div>
				<button type="button" class="btn bg-gradient-info" onclick="btnEditarMar('.$data[$i]['id'].');"><i class="fas fa-edit"></i></button>
				<button type="button" class="btn bg-gradient-danger" onclick="btnEliminarMar('.$data[$i]['id'].');"><i class="fas fa-trash-alt"></i></button>
				<button type="button" class="btn bg-gradient-success" onclick="btnReingresarMar('.$data[$i]['id'].');"><i class="fas fa-plus"></i></button>
			</div>';
		}
	...
	public function registrar()
	{
		...
		if (empty($nombre)) {
			...
			if ($id == "") {
				$data = $this->model->registarMarca($nombre);
				if ($data == "ok") {
					$msg = "si";
				} else if($data == "existe") {
